...
function selectRandom(suspectsArray) {
  ...
  for (let i = 0; i < suspectsArray.length; i++) {
    selectedMystery.push(suspectsArray[Math.floor(Math.random() * suspectsArray.length)])
    break
  }
...
function revealMystery(pickMysteryObject) {
  return `${pickMystery()["suspect"][0]["firstName"]} ${pickMystery()["suspect"][0]["lastName"]} mató al Sr. Boddy usando el ${pickMystery()["weapon"][0]["name"]} en la ${pickMystery()["room"][0]["name"]}`
}
